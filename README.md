# Fernando Augusto Almeida de Abreu

**Full-Stack Developer | 15+ anos de experiência | Ecosistema Microsoft Specialist**

São Paulo, Brazil 🇧🇷 | dev.fernandoaugusto@outlook.com | [LinkedIn](https://www.linkedin.com/in/onandoaugusto)

---

## 👋 Sobre Mim

Desenvolvedor full-stack com 15+ anos de experiência em **Ecosistema Microsoft (.NET, C#, SQL Server)** e tecnologias web modernas. Especialista em traduzir regras de negócio complexas em soluções escaláveis, com atuação profunda em **saúde**, **educação**, **jurídico** e **varejo**.

📌 **Diferenciais:** Infraestrutura Microsoft, mentoria em programação, web crawling avançado, integração de dados complexa.

---

## 🛠 Stacks

| **Front-end** | **Back-end** | **Dados** | **DevOps** | **Patterns & Práticas**
|---|---|---|---|---|
| JavaScript / jQuery | ASP.NET (4.8, 8+) | SQL Server | Azure DevOps | SOLID
| Bootstrap | WebForms / MVC | MySQL / Oracle | GitHub / Git | DRY
| HTML5 / CSS3 | Entity Framework | Redis | TFS / SVN | Singleton
| | Dapper / NHibernate | RegEx / LINQ | | Factory
| | REST APIs | Lambda Expressions | | Dry

---

## 💼 Experiência Recente

>**Analista Desenvolvedor de Sistemas** @ SESI-SENAI SP *(09/2025 – Atual)*<br>
>Sustentação e inovação de sistemas internos pedagógicos e secretariais (outsourcing via Hyti)

>**Analista Desenvolvedor de Sistemas** @ Eicon *(08/2022 – 12/2024)*<br>
>Plataforma Gier (gestão escolar pública) | Soluções web/mobile para famílias, alunos e professores

>**Desenvolvedor de Sistemas** @ AASP – Associação de Advogados de SP *(12/2021 – 07/2022)*<br>
>Web crawling, bots e ferramentas back-office jurídico (outsourcing via Sysmap)

>**Desenvolvedor de Sistemas** @ HC-FMUSP *(04/2021 – 12/2021)*<br>
>Sistemas para operação e atendimento SUS no Hospital das Clínicas da USP

>**Analista Desenvolvedor de Sistemas** @ Versatilis System *(09/2015 – 08/2020)*<br>
>Plataforma ERP em ASP.NET Web Forms para clínicas e estúdios de estética; Prontuários, agendamentos, exames

---

## 🎯 Conhecimentos Setoriais

- **Saúde:** Protocolos TISS/SADT, fluxos de faturamento médico, sistemas hospitalares
- **Educação:** Gestão escolar (Gier), certificação digital de diplomas e históricos escolares
- **Jurídico:** Back-office, crawling, ferramentas especializadas
- **Varejo:** ERP, e-commerce, big data e análise

---

## 📚 Formação

🎓 **Tecnólogo em Sistemas para Internet** – SENAC (4 semestres)  
🌐 **Inglês Intermediário** – Yazigi  
📱 **Xamarin** – FIAP

---

## 🚀 Quer conversar comigo?

📧 [dev.fernandoaugusto@outlook.com](mailto:dev.fernandoaugusto@outlook.com)  
🔗 [LinkedIn](https://www.linkedin.com/in/onandoaugusto)  
📱 [11 97271-6641](tel:11972716641)

---

*Mais de 15 anos `buildando` soluções que fazem diferença. Open to interesting challenges. o/*
